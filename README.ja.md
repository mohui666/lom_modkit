...
PC 上の音声を「ユーザーコンテンツ庫」（メニュー「ファイル → ユーザーコンテンツ庫」）に取り込むと、安定した番号
（例：`user:mohui.battle`）が付き、シナリオステップでは「ユーザー / 公式」のグループに分けて選択できます。シナリオにはこの番号だけを保存します。
書き出し時は現在の Mod が実際に参照している音声だけが同梱されるため、プレイヤーの PC は作者のローカルコンテンツ庫に依存しません。
...
| [Mod パッケージ形式の契約](docs/ja/mod_format.md) | パッケージ構造、43 種のノード、コンパイル規約、ランタイムの挙動 |
