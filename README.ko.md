...
로컬 오디오를 「사용자 콘텐츠 보관함」(메뉴 「파일 → 사용자 콘텐츠 보관함」)으로 가져오면 안정적인 번호
(예: `user:mohui.battle`)를 얻고, 시나리오 단계에서 「사용자 / 공식」 그룹으로 나누어 선택합니다. 시나리오는 번호만 저장합니다.
보낼 때는 현재 Mod가 실제로 참조하는 오디오만 패키징되므로, 플레이어 컴퓨터는 작성자 본인 컴퓨터의 콘텐츠 보관함에 의존하지 않습니다.
...
| [Mod 패키지 형식 규약](docs/ko/mod_format.md) | 패키지 구조, 43종 노드, 컴파일 규약, 런타임 동작 |
