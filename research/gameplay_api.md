...
| 存档 | `SaveSystem.CurrentSlot` / `SetSlot` / `SaveGameData()` / `SaveGameData(string)` | 已实现与 `mod_<id>` 槽绑定的 Host 原子 sidecar；Harmony 明确跟随“保存当前槽”的无参重载，避免双重载歧义；只保存有界 Int32 状态，不修改 GameSave schema |
