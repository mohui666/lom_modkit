...
# 契约 §3.1 全量 48 种节点类型（与 models.NODE_TYPES 一一对应）
ALL_TYPES = [
    "music",
    "sound",
    "scene",
...
class TestAddNode(unittest.TestCase):
    def test_48_types(self):
        story = story_api.new_story()
        self.assertEqual(len(ALL_TYPES), 48, "契约应有 48 种节点类型")
        self.assertEqual(
        ...
        # 2 个开场节点（show+say）+ 48 种各一个 + hide 之后的首个动作节点
        # 触发登场防线自动补 1 个 show
        self.assertEqual(len(story["nodes"]), 2 + 48 + 1, "48 种类型应全部追加进 story（含自动补登场）")
