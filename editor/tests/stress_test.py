...
def main_fn() -> int:
    ...
    # ------------------------------------------------------------------
    # [1b] v3 全量 47 种节点类型：建表 + 渲染不崩（提示行/舞台两条路径都走）
    # ------------------------------------------------------------------
    all_nodes = []
    ...
    # end/goto_scene/death/combat 是终止节点（推演到此为止），排到末尾保证其余节点可达
    terminal_types = ("end", "goto_scene", "death", "combat")
    ...
    print(f"[1b] 全量 47 类型渲染 OK（{len(all_nodes)} 节点，{hints} 个提示行）")
