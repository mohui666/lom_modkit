...
自定义音乐和音效：菜单「文件 → 用户内容库」导入 `.ogg` / `.wav`（≤20MB），会得到稳定编号如 `user:mohui.battle`。在音乐/音效步骤里按「用户 / 官方」分组选择；剧情只保存这个编号。导出时只打入当前 Mod 真正引用的音频，换电脑安装后不依赖作者本机内容库。说明见 `docs/user_content.md`。
