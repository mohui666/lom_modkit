...
Battle Preset、draw/escape コールバック、`reward` 集約ノード、任意商品 Custom Shop、独立 `mod_quest`、任意の永続 Mod 変数は未実装です。`modflags` / `modvars` は Story セッション限定で、`game_flag` は原作に存在する FlagData だけを書きます。戦闘マップ、モデル、AI、アニメーション、機構のカスタムにも対応しません。
