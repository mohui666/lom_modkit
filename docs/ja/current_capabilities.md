...
`combat` は今回の相手 HP、気力、内力、内功、その他能力、才能、行動確率を設定できます。原作が読み取らない三つの奥義欄は削除し、実際に有効な `ultimate_rate` だけを残します。`battle` は原作の味方・敵・中立 roster を別々に再利用し、人数、NPC HP、プレイヤースキルを設定します。前者は Combat win/lose、後者は finish=true の FriendWin/EnemyWin だけを返します。原作 asset は変更せず、独自 Battle Engine でもありません。実機検証は必要です。

低レベルの `enemy`、`battle_skill`、`goto_scene` は互換性と高度な構成向けに維持します。

`combat` の人物は決闘名と四種類の戦闘アニメーションだけを決め、背景は公式 `views` から独立して選択します。体力、能力、スキル、行動確率は自由に設定します。`battle` は味方・敵の陣営、総人数、確認済みの公式名付き人物だけを設定し、名付き人物も総人数に含まれます。旧プリセットと `battle_setup` は削除済みです。永続状態は `mod_campaign_<campaign_id>` 専用スロットに結び付き、原作 GameSave は変更しません。
