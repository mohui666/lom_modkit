...
本文按仓库当前代码描述能力，不把研究计划写成已实现功能。节点的唯一权威集合是 `editor/models.py` 的 `NODE_SCHEMAS`，当前共 46 种。
...
这意味着工具是在编排原版战斗系统，不包含自研 Battle Engine。但它目前也**不等于完整的战斗编排层**：编辑器没有 `combat` / `battle` 高层节点、Battle Preset、经验证的 win/lose/draw/escape 回调、自定义敌军/我方配置编辑器或战后奖励聚合节点。
