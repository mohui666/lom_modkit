...
`combat` 可用原版一對一人物／場景範本並覆寫本次對手 HP、體力、能力、天賦、絕招與行動機率；`battle` 可分別重用原版我方、敵方、中立陣容並設定人數、NPC HP 和玩家技能。前者取得 Combat win/lose，後者只接受 finish=true 的 FriendWin/EnemyWin。原版資產不被修改，這也不是自製戰鬥引擎；新增流程仍需實機驗收。
