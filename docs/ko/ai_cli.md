...
| `add_node(story, node_type, fields=None, after=None) -> dict` | node_type은 45종으로 제한(`models.NODE_TYPES`); fields 키는 NODE_SCHEMAS 합법 필드+공통 필드(id/type/goto)로 제한, 타입은 kind에 따라 느슨하게 검증; 알 수 없는 타입/필드/타입 불일치 → ValueError. id 자동 생성(say1, show2, choice1…), after=노드 id면 그 뒤에 삽입, None이면 끝에 추가. **등장 방어선**: 동작류 노드의 대상 인물이 앞에서 미등장/퇴장 상태이면 그 앞에 자동으로 show 노드를 하나 삽입(§4 규칙 4 참조) |
...
| 오류 메시지(예시) | 출처 | 원인과 처리 |
| --- | --- | --- |
| `未知节点类型: no_such_type（支持 45 种，见 models.NODE_TYPES）` | add_node | 타입명 철자 오류; `models.NODE_TYPES` 또는 규약 §3.1의 45종 사용 |
