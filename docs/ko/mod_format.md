...
이 표가 현재 합법 노드 전부입니다. `combat` / `battle`은 원작 템플릿을 사용하는 고수준 편성입니다. `reward`, `quest_*`는 아직 노드가 아니며 전투 기능은 디컴파일로 확인한 원작 API만 호출합니다.
...
| `custom_shop` | `items`(1~64): book/misc/special 원작 아이템 id, 재고, 선택 MOD/원작 조건. 선택 `discount`(0/1) | 원작 `ShopDatabase` 재고를 일시 교체해 `ShopPanel`을 열고 종료 또는 오류 시 복원합니다. 가격은 원작 일괄 할인만 사용하며 상품별 `price`는 지원하지 않습니다 |
...
- Python API:
  ...
  - `add_node(story, node_type, fields=None, after=None)`: models 기본값으로 노드 추가(52종), 알 수 없는 타입/필드/타입 불일치→ValueError, 노드 id 자동 생성, after로 삽입 위치 지정(노드 id 또는 None=끝). 등장 방어선: 동작류 노드의 대상 인물이 앞에서 등장하지 않았거나 이미 퇴장했으면 그 앞에 자동으로 show 삽입
