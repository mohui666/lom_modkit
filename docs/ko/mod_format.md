...
| `dice` | `max`(1～9999), `header`, `bands`(2～4개) 필수. `bonus`, `bonus_name`, `bonus_status` 선택. 마지막을 제외한 각 구간은 오름차순이며 bonus～max+bonus-1의 도달 가능한 범위 안인 `upper`, 모든 구간은 `text`,`goto` 필수 | `CH_*` 공식 체크포인트를 선택하지 않습니다. Host가 0～max 난수에 고정 bonus를 더하고 낮은 upper부터 결과를 맞춥니다. 마지막 구간은 남은 값을 받습니다. 화면은 원작 `DiceMenuDialog`를 재사용하며 각 구간 문구와 이동 대상은 작성자가 직접 설정합니다 |
