...
- Python API：
  ...
  - `add_node(story, node_type, fields=None, after=None)`：按 models 預設值新增節點（51 種類型），未知類型/欄位/類型不符→ValueError，節點 id 自動產生，after 指定插入位置（節點 id 或 None=末尾）。登場防線：動作類節點的目標人物在前面未登場/已退場時，自動在它前面插入 show
