...
介面只接受 `models.NODE_SCHEMAS` 目前列出的 47 種節點。`combat` 已實作為原版 Combat 模板的 win/lose 編排；不要讓 AI 發明尚不存在的 `battle`、`reward`、`quest_*`。
...
| `add_node(story, node_type, fields=None, after=None) -> dict` | node_type 限 47 種（`models.NODE_TYPES`）；fields 鍵限 NODE_SCHEMAS 合法欄位+通用欄位（id/type/goto），類型按 kind 寬鬆驗證；未知類型/欄位/類型不符 → ValueError。id 自動產生（say1、show2、choice1…），after=節點 id 插到其後、None 追加末尾。**登場防線**：動作類節點的目標人物在前面未登場/已退場時，自動在它前面插一個 show 節點（見 §4 規則 4） |
...
| 錯誤訊息（樣例） | 來源 | 原因與處理 |
| --- | --- | --- |
| `未知节点类型: no_such_type（支持 47 种，见 models.NODE_TYPES）` | add_node | 類型名拼錯；用 `models.NODE_TYPES` 或契約 §3.1 的 47 種 |
