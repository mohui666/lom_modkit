...
| `combat` | 必填 `character`, `win`, `lose`；可自由填写 `display_name`，并直接设置对手 `max_health/health/max_stamina/stamina`、九项属性、`talents`、三格绝招和五类行动概率 | `character` 接受官方 ID 或 `user:` 自定义人物，只决定四类战斗动画；显示名和其余数值始终自由填写，不会从人物自动带入或受其限制。未填显示名时使用中性的“MOD 对手”，不会沿用官方外壳人物名。Host 只覆盖本次 Combat，不修改原版资产；真实 win/lose 回到指定节点 |
