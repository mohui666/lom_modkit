...
| `combat` | 新建节点必填 `character`, `background`, `win`, `lose`；可直接设置对手 HP/气力、内力/内功、四学/战术、抗性/暗器累积、性情/处世/道德/修养、全部实际生效的伤害/骰子/命中/招架补正、`talents` 和五类行动概率 | `talents` 使用从官方 `PlayerTalentData` 实证提取的 115 个 CombatSkill，并按每项真实最大等级校验；不再复用只有脚本引用子集的普通天赋清单。`character` 只决定名称与动画，`background` 独立选择。死字段不暴露；旧 Story 缺少背景时迁移为 `center` |
